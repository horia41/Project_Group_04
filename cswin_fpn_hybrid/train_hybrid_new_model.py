...
def fine_tune_model(model_name, trainloader, testloader, num_classes, epochs=100):
    ...
    for name, param in model.named_parameters():
        ...
        if 'resnet_stem' in name:
           ...
           # bridge, stage3, merge3, stage4, norm, pos_embed,
    ...
    # 2. Main: Cosine Annealing for the rest of the time
    ...
    model_save_path = os.path.join(save_folder, f'threshold_{optimal_t:.2f}_hybrid_Tr2019_Te2022.pth')
...
def load_data():
    ...
    dset_loaders = {
        'train': torch.utils.data.DataLoader(dsets['train'], batch_size=batch_size, shuffle=True, num_workers=4), # remember here to delete drop_last = True
        'test' : torch.utils.data.DataLoader(dsets['test'],  batch_size=batch_size, shuffle=False, num_workers=4),                  # if mixup doesnt work
    }
